...
INGRESS_IDENTITY = re.compile(
    r"^(?P<identity>UlogIngressTopology/"
    r"(?:bounded-mpsc-ring|chunked-mpsc|per-producer-lanes)/"
...
class IngressScheduleError(RuntimeError):
    pass
...
def normalize_listing_row(row: str, row_index: int) -> str:
    stripped = row.strip()
    match = INGRESS_IDENTITY.fullmatch(stripped)
    if not match:
        raise IngressScheduleError(
            f"Ingress benchmark listing row {row_index} has unrecognized identity "
            f"{stripped!r}. Keep UlogIngressTopology names stable and retry."
        )
...
def read_listing(executable: Path) -> list[str]:
    try:
        ...
        raise IngressScheduleError(
            f"Ingress benchmark listing exceeded {LISTING_TIMEOUT_SECONDS} seconds. "
            "Check benchmark initialization for accidentally executed workloads and retry."
        ) from error
    except OSError as error:
        raise IngressScheduleError(
            f"Unable to execute Ingress benchmark '{executable}': {error}. "
            "Build the Ingress benchmark executable and retry."
        ) from error
    if result.returncode != 0:
        details = result.stderr.strip() or result.stdout.strip() or "no diagnostic output"
        raise IngressScheduleError(
            f"Ingress benchmark listing exited with {result.returncode}: {details}"
        )
...
def validate_listing(actual: list[str]) -> int:
    ...
    if len(actual) != len(expected):
        raise IngressScheduleError(
            f"Ingress benchmark listing has {len(actual)} row(s); expected "
    ...
    if actual != expected:
        ...
        raise IngressScheduleError(
            f"Ingress benchmark listing row {mismatch_index} is "
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Ulog's controlled ingress topology registration schedule."
    )
...
def main() -> int:
    ...
    except IngressScheduleError as error:
        print(f"ingress schedule validation failed: {error}", file=sys.stderr)
        return 1
    print(
        f"Validated six-permutation Ingress schedule for {row_count} benchmark row(s)"
    )
    return 0
