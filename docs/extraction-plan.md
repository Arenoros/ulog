...
По завершению каждого этапа обязательная цепочка:

1. **Сборка** через `ulog/build-helper.bat cmake --build ulog/build`.
2. **Прогон тестов** через `ulog/build/tests/ulog-tests.exe` — все зелёные.
3. **Code review** — если сборка + тесты OK. Результаты сохранить в `ulog/docs/review/NN-<phase>.md` (NN — порядковый номер рецензии).
4. **Устранение критичных замечаний** + добавление недостающих тестов.
5. **Повторная сборка + прогон** — подтвердить, что ничего не сломано.
6. **Commit** (Conventional Commits, terse, why > what).
7. **Самостоятельный переход к следующему этапу** — без дополнительного разрешения.

Правило: не переходить к следующему этапу с зафейленной сборкой, падающими тестами или нерасшитыми critical-замечаниями из рецензии. Non-critical / долгосрочные пункты уходят в `docs/BACKLOG.md`.
